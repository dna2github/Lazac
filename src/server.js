...
const i_env = {
   debug: !!process.env.LAZAC_DEBUG,
   server: {
      host: process.env.LAZAC_HOST || '127.0.0.1',
      port: parseInt(process.env.LAZAC_PORT || '8080'),
      staticDir: process.env.LAZAC_STATIC_DIR?i_path.resolve(process.env.LAZAC_STATIC_DIR):null,
      httpsCADir: process.env.LAZAC_HTTPS_CA_DIR?i_path.resolve(process.env.LAZAC_HTTPS_CA_DIR):null,
   },
};
...
server.listen(i_env.server.port, i_env.server.host, () => {
   console.log(`Lazac server is listening at ${i_env.server.host}:${i_env.server.port}`);
});
